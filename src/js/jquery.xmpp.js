    ...
    $.xmpp ={
		...
		*/
		getVcard: function(callback){
			//Some XMPP server send a blank list. Use onIq in these cases
            //var msg = "<iq type='get' "+((to)?"to='"+to+"'":"")+"><vCard xmlns='vcard-temp'/></iq>";
            var msg = "<iq type='get'><vCard xmlns='vcard-temp'/></iq>";
            this.sendCommand(msg, function(data){
				var vcard = {};
                var vc = $(data).find("vcard");
				vcard.nickname = vc.find('nickname').text();
                if(callback)
					callback(vcard);
            });
        },
